...
function looksLikeListicle(title: string) { return /\b(best|top\s+\d+|ultimate guide|complete guide|\d+\s+(?:best|top|ways|tools|agents|platforms))\b/i.test(title); }
...
export async function researchStrategy(request: StrategyRequest) {
  ...
  for (const { plan, results } of sets) for (const result of results) { if (!result.title || !validUrl(result.url) || looksLikeListicle(result.title)) continue; const url = new URL(result.url!).href.split("#")[0]; const excerpt = [result.summary, ...(result.highlights ?? [])].filter(Boolean).join("\n").trim(); if (seen.has(url) || excerpt.length < 80) continue; seen.add(url); external.push({ id: "", title: result.title, url, domain: new URL(url).hostname.replace(/^www\./, ""), purpose: plan.purpose, publishedDate: result.publishedDate, excerpt: excerpt.slice(0, MAX_SOURCE_CHARS) }); }
